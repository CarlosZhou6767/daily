...
/**
 * 幸运转盘抽奖页面
 * Canvas 绘制转盘，CSS 动画控制旋转，支持防重复点击锁
 * 展示奖品列表、中奖记录，消耗积分参与抽奖
 */
...
/**
 * 处理抽奖点击
 * 防重复点击 → 校验积分 → 请求后端 → 计算旋转角度 → 播放动画 → 显示结果
 */
async function handleSpin() {
  ...
  try {
    ...
    if (prizeIndex === -1) {
      ...
      const offset = ((270 - prizeCenterAngle) % 360 + 360) % 360
      currentRotation.value = 360 * 5 + offset
    }

    // 等待转盘动画结束后显示结果
    spinTimer = setTimeout(async () => {
      spinning.value = false
      resetTimer = setTimeout(() => { currentRotation.value = currentRotation.value % 360 }, 100)
