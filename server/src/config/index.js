...
const DEFAULT_JWT_SECRET = 'DEV_ONLY_CHANGE_ME_IN_PRODUCTION';
let jwtSecret = getEnv('JWT_SECRET', DEFAULT_JWT_SECRET, true);

if (jwtSecret === DEFAULT_JWT_SECRET) {
  const machineFallback = crypto.createHash('sha256').update(require('os').hostname() + process.cwd()).digest('hex');
  jwtSecret = machineFallback;
  console.warn('[安全警告] 正在使用默认JWT密钥，已自动替换为基于机器名的备用密钥。生产环境请设置环境变量 JWT_SECRET！');
}
...
const wechatAppId = getEnv('WECHAT_APP_ID', '');
const wechatAppSecret = getEnv('WECHAT_APP_SECRET', '');
